...
def main():
    """
    CLI interface for the CASE data pipeline
    """
    import argparse
    import sys
    
    parser = argparse.ArgumentParser(description='CASE Data Pipeline')
    parser.add_argument(
    ...
    parser.add_argument(
        "--load",
        help="Load CASE data from JSON file"
    )
    ...
    if args.load:
        pipeline.load_from_json(args.load)
        
    execution_count = 0
    
    if args.create_investigation:
        inv_id, title, desc = args.create_investigation
        res_id = pipeline.add_investigation(inv_id, title, desc)
        print(f"Created investigation: {res_id}")
        execution_count += 1
        
    if args.add_evidence:
        ev_id, inv_id, file_path = args.add_evidence
        res_id = pipeline.add_evidence(ev_id, inv_id, file_path)
        print(f"Added evidence: {res_id}")
        execution_count += 1
        
    if args.add_observable:
        obs_id, ev_id, obs_type, value = args.add_observable
        res_id = pipeline.add_observable(obs_id, ev_id, obs_type, value)
        print(f"Added observable: {res_id}")
        execution_count += 1
        
    if args.save:
        pipeline.save_to_json(args.save)
        execution_count += 1
        
    if execution_count == 0 and not args.load:
        # Default behavior if no arguments
        print("No commands provided. Running default example...")
        inv_id = pipeline.add_investigation(
            "inv_001", 
            "Suspicious Network Activity", 
            "Investigation of potential APT activity"
        )
        print(f"Example investigation created: {inv_id}")
